...
namespace Microsoft.Azure.SpatialAnchors.Unity.Examples
{
    public abstract class DemoScriptBase : InputInteractionBase
    {
        ...
        // ガイドのパネル
        protected GameObject feedbackPanel;
        // 有効期限日数のデフォルト値
        protected int EXPIRATION_DAYS_DEFAULT = 7;
        // 有効期限日数の入力値
        ...
        public override void Start()
        {
            ...
            feedbackPanel = XRUXPicker.Instance.GetFeedbackPanel();
            if (feedbackPanel == null)
            {
                Debug.Log($"{nameof(feedbackPanel)} not found in scene by XRUXPicker.");
                Destroy(this);
                return;
            }
        ...
        protected virtual async Task SaveCurrentObjectAnchorToCloudAsync()
        {
            ...
            cloudAnchor.Expiration = DateTimeOffset.Now.AddDays(expirationDays);

            Debug.Log("cloudAnchor.Expiration=" + cloudAnchor.Expiration);
            ...
            while (!CloudManager.IsReadyForCreate)
            {
                ...
                feedbackBox.text = $"デバイスを移動して多くの環境データをキャプチャしてください: {createProgress:0%}";
            ...
            try
            {
                ...
                {
                    Debug.Log("＊＊＊ id cloudAnchor=" + cloudAnchor.Identifier + ", currentCloudAnchor=" + currentCloudAnchor.Identifier);
...
//                spawnedObjectMat = spawnedObject.GetComponent<MeshRenderer>().material;
                ...
                // 伝言板名のデフォルト設定
                Text[] txt = spawnedObject.GetComponentsInChildren<Text>();
                for (int i = 0; i < txt.Length; i++)
                {
                    Debug.Log(txt[i].name);
                    if (txt[i].name == "TextBoardName")
                    {
                        txt[i].text = PlayerPrefs.GetString("HandleName") + "さんの伝言板";
                        AnchorsMsgBoard.inputBoardNameValue = PlayerPrefs.GetString("HandleName") + "さんの伝言板";
                        i = txt.Length;
                    }
                }
                feedbackBox.text = "置き場所が決定したらチェックしてください。";
                XRUXPicker.Instance.GetDemoButtons()[0].gameObject.SetActive(true);
            }
            else
            {
        ...
        public GameObject AnchoredObjectPrefab { 
            get { return anchoredObjectPrefab; }
            set { anchoredObjectPrefab = value; }
        }
